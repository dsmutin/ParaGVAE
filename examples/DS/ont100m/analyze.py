#!/usr/bin/env python3
"""Where the ONT 100M GCN puts a contig in the wrong majority genome."""
...
def main() -> None:
    """Train on k-mer and depth, which are stored in place of a separate VAE latent."""
    graph = load_vaegbin("ont100m")
    if not graph["vae_equals_raw"]:
        raise RuntimeError("ONT 100M was expected to store the same matrix as VAE and raw features")
